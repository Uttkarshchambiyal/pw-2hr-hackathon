...
export const PricingCard = ({
  planName, description, price, features, buttonText, isPopular = false, buttonVariant = 'primary', currencySymbol = '$'
}: PricingCardProps) => {
  const cardClasses = `
  ...
  return (
    <div className={cardClasses.trim()}>
      ...
      <div className="my-6 flex items-baseline gap-2">
        <span className="text-[48px] font-extralight text-foreground font-display">{currencySymbol}{price}</span>
      </div>
  ...
  return (
    <div className="bg-background text-foreground min-h-screen w-full overflow-x-hidden relative">
      ...
      {onBack && (
        <button
          onClick={onBack}
          className="absolute top-8 left-8 z-50 flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900/50 backdrop-blur-md border border-white/10 text-white hover:bg-zinc-800/80 transition-all group cursor-pointer"
        >
      ...
      <main className="relative z-10 w-full min-h-screen flex flex-col items-center justify-center px-4 py-8">
        <div className="w-full max-w-5xl mx-auto text-center mb-14 mt-16 md:mt-0">
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-6 justify-center items-center w-full max-w-5xl">
