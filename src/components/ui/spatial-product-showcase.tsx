...
const ProductVisual = ({ data, isLeft }: { data: ProductData; isLeft: boolean }) => (
  <motion.div layout="position" className="relative group shrink-0">
    ...
    {/* Image Container */}
    <div className="relative h-80 w-80 md:h-[450px] md:w-[450px] rounded-full border border-white/5 shadow-2xl flex items-center justify-center overflow-hidden bg-black/20 backdrop-blur-sm z-10 transition-transform duration-500 hover:scale-105">
      <motion.div
        animate={{ y: [-10, 10, -10] }}
      ...
      >
        <AnimatePresence mode="wait">
          <motion.img
            ...
            src={data.image}
            alt={`${data.title}`}
            variants={ANIMATIONS.image(isLeft) as any}
            initial="initial"
            animate="animate"
            exit="exit"
            className="w-full h-full object-cover rounded-full drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)] p-4 mix-blend-screen"
            draggable={false}
          />
        </AnimatePresence>
      </motion.div>
    ...
    >
      <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-zinc-500 bg-zinc-950/80 px-4 py-2 rounded-full border border-white/5 backdrop-blur z-20">
...
const ProductDetails = ({ data, isLeft }: { data: ProductData; isLeft: boolean }) => {
  ...
  return (
    ...
    >
      <motion.h2 variants={ANIMATIONS.item as any} className="text-sm font-bold uppercase tracking-[0.2em] text-zinc-500 mb-2">
        {data.label} Earbud
      </motion.h2>
      <motion.h1 variants={ANIMATIONS.item as any} className="text-4xl md:text-5xl font-bold tracking-tight mb-2 text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-500">
        {data.title}
      </motion.h1>
      <motion.p variants={ANIMATIONS.item as any} className={`text-zinc-400 mb-8 max-w-sm leading-relaxed ${isLeft ? 'mr-auto' : 'ml-auto'}`}>
        {data.description}
      </motion.p>

      {/* Feature Grid */}
      <motion.div variants={ANIMATIONS.item as any} className="w-full space-y-6 bg-zinc-900/40 p-6 rounded-2xl border border-white/5 backdrop-blur-sm z-10">
      ...
      {/* Battery */}
      <motion.div variants={ANIMATIONS.item as any} className={`mt-6 flex items-center gap-3 text-zinc-500 ${flexDirClass}`}>
        <Battery size={16} />
...
export default function EarbudShowcase({ onBack }: EarbudShowcaseProps) {
    ...
    <div className="relative min-h-screen w-full bg-black text-zinc-100 overflow-hidden selection:bg-zinc-800 flex flex-col items-center justify-center">
      ...
      {onBack && (
        <motion.button
          ...
          onClick={onBack}
          className="absolute top-8 left-8 z-50 flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900/50 backdrop-blur-md border border-white/10 text-zinc-300 hover:text-white hover:bg-zinc-800/80 transition-all group cursor-pointer"
        >
      ...
      <main className="relative z-10 w-full px-6 py-8 flex flex-col justify-center min-h-screen max-w-7xl mx-auto">
        <motion.div
          layout
